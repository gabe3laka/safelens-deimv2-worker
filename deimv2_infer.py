...
def _load_model():
    ...
    try:
        ...
        # DEIMv2 ships a custom HF config/model; trust_remote_code is required so
        # the repo's modeling/processing code is loaded rather than the stock
        # transformers classes (which raises ModuleNotFoundError for the custom
        # processor / model). See /debug/model-load for the full structured error.
        ...
        log.error("[deimv2] model load FAILED: %s: %s\n%s", type(exc).__name__, exc, tb)
        # Re-raise so the caller (warmup / /debug/model-load) records the full
        # traceback in worker state and surfaces it via /debug/startup.
        raise
