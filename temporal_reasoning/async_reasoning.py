...
A newer trigger while a session job is in flight is simply skipped -- the running
job already uses the latest stored memory, so stale work is never queued.
"""
...
def _max_async_jobs() -> int:
    return max(1, _int_env("TEMPORAL_REASONING_MAX_ASYNC_JOBS", 2))


def _min_interval_ms() -> int:
    return max(_int_env("TEMPORAL_REASONING_TRIGGER_MIN_INTERVAL_MS", 5000),
              _int_env("REASONER_MIN_INTERVAL_MS", 5000))
...
def maybe_trigger(session_id: Optional[str], *, reasons: List[str],
    ...
    Status: disabled | not_triggered | throttled | triggered | running
    """
    vlm = _vlm()
    if not vlm.enabled():
        return "disabled"
