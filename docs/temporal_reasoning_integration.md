...
`gemini` (default, API-only, no weights), `mock` (no weights; deterministic; used by tests + CPU integration),
`disabled`. `qwen_vl` / `deepseek_vl2` modes were removed and degrade to `unavailable`.
