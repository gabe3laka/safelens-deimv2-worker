...
## 6. Enable the Gemini reasoner

```env
VLM_REASONER_ENABLED=true    # event-driven; NEVER per-frame; never blocks /detect
REASONER_MODE=gemini          # gemini (default) | mock | disabled
GEMINI_API_KEY=your-api-key-here   # set at deploy time, NEVER bake into image
...
# QWEN_VL_DEEP_* remain declared as inert future/deep/offline placeholders:
QWEN_VL_DEEP_MODEL_ID=Qwen/Qwen2.5-VL-7B-Instruct
QWEN_VL_DEEP_ENABLED=false
```

- `REASONER_MODE=mock` → CPU, weight-free draft contract (integration without API key).
- `REASONER_MODE=qwen_vl` or `deepseek_vl2` → degrades to `reasoner_status=unavailable`; no transformers loaded.
- VLM output is always an **AI draft** (`requires_human_review=true`,
  `should_alert=false`); it never becomes the safety authority.
- Gemini is API-only: no weights, no GPU memory for the reasoner.
